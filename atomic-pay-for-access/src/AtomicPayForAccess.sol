...
/// @title AtomicPayForAccess
/// @notice Combines SplitClaimHashlock escrow with X402Cap capability minting.
///         One transaction: pay → reveal preimage → settle fees → mint access credential.
contract AtomicPayForAccess {
    // ─── Errors ───────────────────────────────────────────────
    ...
    /// @notice Deposit ETH into hashlock escrow and pre-register capability parameters.
    ///         The capability will be minted to the depositor when the preimage is revealed.
    function depositAndConfigure(
        ...
        emit DealDeposited(msg.sender, id, beneficiary, uint96(msg.value), deadline, hash, operator, feeBps);
    }
